 ...
 * See the createMembrane function below for details.
 *
 ...
 *
 * Usage:
 * var membrane = createMembrane(wetObject)
 ...
 * revocable proxies.
 */
export function createMembrane(initWetTarget) {
  /****************************************************************************
      Design notes:
  ...
  // is used
  var dryToWetMaker = function (dryToWetCache, wetToDryCache, dryToWetRef, wetToDryRef, revokeFnsCache) {
    ...
    return function (dryTarget) {
      ...
      const { proxy, revoke } = Proxy.revocable(wetShadowTarget, {
        getOwnPropertyDescriptor: function (wetShadowTarget, name) {
          if (revoked) throw new Error("revoked");

          // no-invariant case:
          if (Object.isExtensible(wetShadowTarget) && isConfigurable(dryTarget, name)) {
        ...
        defineProperty: function (wetShadowTarget, name, wetDesc) {
          ...
          // no-invariant case:
          if (Object.isExtensible(wetShadowTarget) && wetDesc.configurable === true) {
        ...
        set: function (wetShadowTarget, name, val, wetReceiver) {
          ...
          if (isConfigurable(dryTarget, name)) {
            ...
            return dryToWet(Reflect.set(dryTarget, name, wetToDry(val), wetToDry(wetReceiver)));
          }
  ...
  var dryToWetRevokeFnsCache = new RevokeFnCache();
  var wetToDryRevokeFnsCache = new RevokeFnCache();
  ...
  dryToWetRef.val = dryToWetMaker(dryToWetCache, wetToDryCache, dryToWetRef, wetToDryRef, dryToWetRevokeFnsCache);

  // note the reversed order of wetToDry and dryToWet:

  wetToDryRef.val = dryToWetMaker(wetToDryCache, dryToWetCache, wetToDryRef, dryToWetRef, wetToDryRevokeFnsCache);

  return {
    membrane: wetToDryRef.val(initWetTarget),
    revoke: function () {
      dryToWetCache = null;
      wetToDryCache = null;
      revoked = true;
      dryToWetRevokeFnsCache.revokeAll();
      wetToDryRevokeFnsCache.revokeAll();
    },
  };
} // end createMembrane
